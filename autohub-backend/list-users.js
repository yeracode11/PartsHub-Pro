 ...
 *   node list-users.js
 *   или
 *   DB_PASSWORD=your_password node list-users.js
 * 
 * Требуется: переменные окружения из .env файла или передача через командную строку
 */
...
async function listUsers() {
  ...
  try {
    ...
    // Получаем список всех пользователей с информацией об организации
    const query = `
      SELECT 
        u.id,
        u.email,
    ...
    if (error.code === 'ECONNREFUSED') {
      ...
      console.error('   - Доступность хоста и порта');
